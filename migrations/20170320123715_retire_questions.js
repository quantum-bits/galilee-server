...
exports.up = function (knex, Promise) {
    return knex.schema.createTableIfNotExists('direction', table => {
        ...
        table.integer('practiceId').unsigned().notNullable().references('practice.id').onDelete('CASCADE');
        table.integer('readingId').unsigned().references('reading.id').onDelete('CASCADE');
        table.integer('readingDayId').unsigned().references('readingDay.id').onDelete('CASCADE');
    }).then(() => {
        ...
        return knex.schema.createTableIfNotExists('newStep', table => {
            ...
            table.integer('directionId').unsigned().notNullable().references('direction.id').onDelete('CASCADE');
        })
    }).then(() => {
        // Migrate 'application'
        return knex
            .select('id', 'seq', 'readingId', 'practiceId')
            .from('application')
            .map(applicationRow => {
                return knex('direction').returning('id').insert({
                    ...
                    return knex.select('seq', 'description')
                        .from('step')
            ...
            return knex
                ...
                .map(readingDayRow => {
                    ...
                    return knex
                        .select('seq', 'text')
                        .from('question')
                        .where('readingDayId', readingDayRow.id).then(rows => {
                            if (rows.length > 0) {
        ...
        return knex
            .schema.table('journalEntry', table => {
                table.dropForeign('stepId');
